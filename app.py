...
# Function to truncate product name - MODIFIED TO ADD DEFAULT LENGTH
...
def autocomplete():
    """
    Handles autocomplete requests for product names.
    """
    query = request.args.get('query', '')
    if query:
        # Get all unique product names
        all_product_names = train_data['Name'].unique()

        # Filter for names that start with the query (or contain it, depending on preference)
        # Using a case-insensitive search
        suggestions = [name for name in all_product_names if query.lower() in name.lower()]

        # Limit the number of suggestions to keep the list clean
...
if __name__ == '__main__':
    # You should only have one of these blocks.
    # The flask app will run with all the routes defined above.
    app.run(debug=True)